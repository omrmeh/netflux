...
{
    qDebug() << "fetching urls at column number " << column << "..." << endl;
    urls.clear();

    for(int i=0; i<this->rowCount(); i++)
         urls[this->index(i,0/*colonne de l'id dans le modèle*/).data().toInt()]=this->index(i,column/*colonne de l'url dans le modèle*/).data().toString();
}
